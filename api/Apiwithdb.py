...
@app.get("/loan_data/gender/")
async def read_loan_data_by_gender(gender: str, db: mysql.connector.connection.MySQLConnection = Depends(get_db)):
    cursor = db.cursor(dictionary=True)
    query = "SELECT * FROM loan_data WHERE Gender = %s"
    ...
    if result:
        ...
        raise HTTPException(status_code=404, detail=f"No data found for Gender: {gender}")

@app.get("/loan_data/marital_status/")
async def read_loan_data_by_marital_status(married: bool = Query(None), db: mysql.connector.connection.MySQLConnection = Depends(get_db)):
    cursor = db.cursor(dictionary=True)
    ...
    if result:
        ...
        raise HTTPException(status_code=404, detail="No data found for the specified marital status")

@app.post("/loan_data/gender")
async def create_loan_data_by_gender(gender: str, db: mysql.connector.connection.MySQLConnection = Depends(get_db)):
    try:
        cursor = db.cursor()
        
        # Generate a unique Loan_ID (you may use a more sophisticated method)
        import uuid
        loan_id = str(uuid.uuid4())

        # Insert data into MySQL with only the Gender specified
        query = "INSERT INTO loan_data (Loan_ID, Gender) VALUES (%s, %s)"
        cursor.execute(query, (loan_id, gender))
